...
def field_calc(run,otype,outnum,Nx=256,Ny=256,Nz=256,Lx=2*np.pi,Ly=2*np.pi,Lz=2*np.pi):
    ...
    if 'wz' in otype:
        ...
        vx = (np.fromfile(path+'vx.'+outnum+'.out').reshape(shape))*(np.fromfile(path+'Lvx.'+outnum+'.out').reshape(shape))
        vy = (np.fromfile(path+'vy.'+outnum+'.out').reshape(shape))*(np.fromfile(path+'Lvy.'+outnum+'.out').reshape(shape))
        vz = (np.fromfile(path+'vz.'+outnum+'.out').reshape(shape))*(np.fromfile(path+'Lvz.'+outnum+'.out').reshape(shape))
        final = vx+vy+vz
        final.tofile(path+'bdiss.'+outnum+'.out')
    elif 'wx' in otype:
