...
!*****************************************************************
      SUBROUTINE entrans_bbu(a,b,nmb)
!-----------------------------------------------------------------
!
! Computes the magnetic energy transfer from the u.b.grad(b) term in 
! Fourier space in 2D MHD. The output is written 
! to a file by the first node.
!
! Parameters
!     a  : streamfunction
!     b  : Poisson bracket of the vector potential and current
!     nmb: the extension used when writting the file
!
      USE fprecision
      USE commtypes
...
!
      CALL MPI_REDUCE(Ek,Ektot,n/2+1,MPI_DOUBLE_PRECISION,MPI_SUM,0, &
                      MPI_COMM_WORLD,ierr)
      IF (myrank.eq.0) THEN
         OPEN(1,file='mtransfer_bbu.' // nmb // '.txt')
         WRITE(1,20) Ektot
   20    FORMAT( E23.15 ) 
         CLOSE(1)
      ENDIF
      ...
      RETURN
      END SUBROUTINE entrans_bbu

!*****************************************************************
      SUBROUTINE entrans_bub(a,b,nmb)
!-----------------------------------------------------------------
!
! Computes the magnetic energy transfer from the b.(b.grad(u)+u.grad(b)) terms in 
! Fourier space in 2D MHD. The output is written 
! to a file by the first node.
!
! Parameters
!     a  : current
!     b  : Poisson bracket of the streamfunction and vector potential
!     nmb: the extension used when writting the file
!
      USE fprecision
      USE commtypes
...
!
      CALL MPI_REDUCE(Ek,Ektot,n/2+1,MPI_DOUBLE_PRECISION,MPI_SUM,0, &
                      MPI_COMM_WORLD,ierr)
      IF (myrank.eq.0) THEN
         OPEN(1,file='mtransfer_bub.' // nmb // '.txt')
         WRITE(1,20) Ektot
   20    FORMAT( E23.15 ) 
         CLOSE(1)
      ENDIF
